...
function ListaReceber() {
    ...
    const fetchCadastros = async () => {
        try {
            const response = await instance1.get(`/receber`);
    ...
    const searchFilters = () => {
        ...
        if (tempSearchTerm.trim() !== '') {
            filteredData = filteredData.filter(item => {
                const itemData = `${item.cliente} ${item.empresa} ${item.valorReceber} ${item.vencimento}`.toUpperCase();
