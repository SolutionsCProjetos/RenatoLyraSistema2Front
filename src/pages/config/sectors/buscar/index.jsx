...
function SearchSetor() {
    ...
    const handleConfirmDelete = async () => {
        if (cadastroIdToDelete !== null) {
            try {
                const response = await instance1.delete(`/setor/${cadastroIdToDelete}`);
    ...
    const searchFilters = () => {
        ...
        if (tempSearchTerm.trim() !== '') {
            filteredData = filteredData.filter(item => {
                const itemData = `${item.atividade} ${item.createdAt} ${item.usuario.nome}`.toUpperCase();
            ...
            <div className="grid grid-cols-1 gap-4 max-w-full mx-3">
                ...
                <table className="min-w-full">
                    <thead className="bg-hearder-blue text-white">
                        <tr>
                            <th className="px-4 py-2  border border-black">Setor</th>
                    ...
                    <tbody>
                        {currentCadastros.map(cadastro => (
                            <tr key={cadastro.id}>
                                <td className="border px-4 py-2 border-black">{cadastro.setor}</td>
                                <td className="border px-4 py-2 border-black">
                                    <Link to={`/irl/setor/${cadastro.id}`}>
                                        <BotaoEditarT />
                                    </Link>
                ...
                <div className="flex justify-end">
                    <Link to={`/irl/setor`} className="block text-sm font-semibold">
