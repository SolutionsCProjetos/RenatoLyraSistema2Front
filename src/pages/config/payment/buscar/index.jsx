...
export default function SearchPayment() {
    ...
    const handleConfirmDelete = async () => {
        if (cadastroIdToDelete !== null) {
            try {
                const response = await instance1.delete(`/pagamento/${cadastroIdToDelete}`);
        ...
        <div className="bg-powder-Blue bg-opacity-20 backdrop-filter backdrop-blur-md rounded-lg shadow-xl mb-16 sm:mb-0">
            <div className="grid grid-cols-1 gap-4 max-w-full mx-3">
                ...
                <table className="min-w-full">
                    ...
                    <tbody>
                        {currentCadastros.map(cadastro => (
                            <tr key={cadastro.id}>
                                <td className="border px-4 py-2 border-black">{cadastro.formasPagamento}</td>
                                <td className="border px-4 py-2 border-black">
                                    <Link to={`/irl/payment/${cadastro.id}`} >
                                        <BotaoEditarT />
                                    </Link>
                ...
                <div className="flex justify-end">
                    <Link to={`/irl/payment`} className="block text-sm font-semibold">
