...
export default function InputPayment() {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
          ...
          const response = await instance1.post("/pagamento", TipoData);
            ...
            <form >
                <div className="bg-slate-200 bg-opacity-10 backdrop-filter backdrop-blur-md rounded-lg shadow-xl w-full h-full">
                    <div className="grid grid-cols-1 gap-4 ml-2">
                        <h2 className="text-2xl text-black underline col-start-1 row-start-1">Pagamento</h2>
                    </div>
                    <div className="grid grid-cols-1 gap-4 mx-2">
                        <CheckList
                            types="text"
                            descricao="Tipo:"
                            placeholderInput="Pix"
                            value={tipo}
