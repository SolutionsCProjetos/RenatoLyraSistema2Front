...
import { BotaoFiltrar, BotaoLimpar, BotaoNovo, BotaoEditarT, BotaoDeletar } from "../../../../components/Butons";
...
export default function SearchPayment() {
    ...
    const handleConfirmDelete = async () => {
        if (cadastroIdToDelete !== null) {
            try {
                const response = await instance1.delete(`/categoria/${cadastroIdToDelete}`);
    ...
    const fetchCadastros = async () => {
        try {
            ...
            showErrorMessage("Erro ao carregar os setores.");
        }
    };
    ...
    const searchFilters = () => {
        ...
        const categoria = tempSearchTerm;
        fetchRelatorio(categoria);
    };
            ...
            <div className="grid grid-cols-1 gap-4 max-w-full mx-3">
                ...
                <table className="min-w-full">
                    <thead className="bg-hearder-blue text-white">
                        <tr>
                            <th className="px-4 py-2  border border-black">Categoria</th>
                    ...
                    <tbody>
                        {currentCadastros.map(cadastro => (
                            <tr key={cadastro.id}>
                                <td className="border px-4 py-2 border-black">{cadastro.categoria}</td>
                                <td className="border px-4 py-2 border-black">
                                    <Link to={`/irl/categoria/${cadastro.id}`} className="text-blue-900 hover:text-blue-950 mx-1.5">
                                        <BotaoEditarT />
                                    </Link>
                ...
                <div className="flex justify-end">
                    <Link to={`/irl/categoria`} className="block text-sm font-semibold">
