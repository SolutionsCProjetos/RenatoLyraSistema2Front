...
export default function SearchSetor({ onSelectSetor, initialSetor, setorSel }) {
    ...
    const [setorSelecionado, setSetorSelecionado] = useState(!!initialSetor);
    ...
    const handleSelectChange = (event) => {
      ...
      if (setorSelecionado) {
        setQuery(setorSelecionado.setor);
        onSelectSetor(setorSelecionado.id); // Passa o ID do setor
        setSetorSelecionado(true);
      }
    };
    ...
    return (
      <div className="flex items-center">
        <div className="flex items-center mt-5 relative">
          <span className="mr-2 absolute -mt-16 text-sm font-medium text-slate-700">{setorSel}</span>
          <select
            value={setorSelecionado ? setores.find((setor) => setor.setor === query)?.id : ""}
