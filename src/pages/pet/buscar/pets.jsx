...
export default function ListaReceber() {
  ...
  const searchFilters = () => {
    ...
    if (tempSearchTerm.trim() !== "") {
      filteredData = filteredData.filter((item) => {
        const itemData =
          `${item.cliente} ${item.nomePet} ${item.raca} ${item.dataNascimento}`.toUpperCase();
        ...
        if (error.response) {
          showErrorMessage(
            error.response?.data?.message ||
              "Erro ao tentar a conexão com o banco!"
          );
        } else {
        ...
        <div className="bg-white bg-opacity-10 backdrop-filter backdrop-blur-md rounded-lg shadow-xl">
          <div className="grid grid-cols-1 gap-3 mx-4">
            ...
            <div className="w-full">
              <div
                className="overflow-y-auto max-h-[32.6rem]"
                style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}
              >
                <table className="min-w-full border table-fixed">
                  <thead className="sticky -top-1 bg-hearder-blue text-white">
                    <tr>
                      <th className="px-2 py-2 border border-black hidden sm:table-cell">
                        <Link>Id</Link>
                      </th>
                      <th className="px-2 py-2 border border-black">
                        <Link>Dono</Link>
                      </th>
                      <th className="px-2 py-2 border border-black hidden md:table-cell">
                        <Link>Nome Pet</Link>
                      </th>
                      <th className="px-2 py-2 border border-black">
                        <Link>dataNascimento</Link>
                      </th>
                      <th className="px-2 py-2 border border-black hidden md:table-cell">
                        <Link>Sexo</Link>
                      </th>
                  ...
                  <tbody>
                    {currentCadastros.map((cadastro) => (
                      <tr
                        key={cadastro.id}
                        className={`border-b ${
                          cadastro.status === "Atrasado" ? "bg-red-400" : ""
                        }`}
                      >
                        <td className="px-2 py-1.5 border border-black hidden sm:table-cell">
                          {cadastro.id}
                        </td>
                        <td className="px-2 py-1.5 border border-black whitespace-nowrap overflow-hidden text-ellipsis max-w-20 sm:max-w-32">
                          {cadastro.cliente}
                        </td>
                        <td className="px-2 py-1.5 border border-black hidden  whitespace-nowrap overflow-hidden text-ellipsis max-w-20 md:table-cell sm:max-w-32">
                          {cadastro.nomePet}
                        </td>
                        <td className="px-2 py-1.5 border border-black">
                          {cadastro.dataNascimento}
                        </td>
                        <td className="px-2 py-1.5 border border-black hidden md:table-cell">
                          {cadastro.sexo}
                        </td>
                        <td className="px-2 py-1.5 border border-black ">
                          <Link
                            to={`/irl/pet/${cadastro.id}`}
