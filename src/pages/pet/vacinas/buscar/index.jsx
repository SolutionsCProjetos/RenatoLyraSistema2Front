...
export default function SearchVacinas() {
    ...
    const handleConfirmDelete = async () => {
        if (cadastroIdToDelete !== null) {
            try {
                const response = await instance1.delete(`/vacina/${cadastroIdToDelete}`);
    ...
    const searchFilters = () => {
        ...
        if (tempSearchTerm.trim() !== '') {
            filteredData = filteredData.filter(item => {
                const itemData = `${item.nomeVacina}`.toUpperCase();
    ...
    return (
        <div className="flex justify-center items-center ">
            <div className="flex flex-col mx-4 items-center w-auto">
                <div className="bg-slate-200 bg-opacity-10 backdrop-filter backdrop-blur-md rounded-lg shadow-xl sm:mb-0">
                    <div className="grid grid-cols-1 gap-4 max-w-full mx-3">
                        <h2 className="text-2xl text-black underline col-start-1 row-start-1">Lista das vacinas</h2>
                        ...
                        <table className="min-w-full">
                            <thead className="bg-hearder-blue text-white">
                                <tr>
                                    <th className="px-4 py-2  border border-black">Vacina</th>
                            ...
                            <tbody>
                                {currentCadastros.map(cadastro => (
                                    <tr key={cadastro.id}>
                                        <td className="border px-4 py-2 border-black">{cadastro.nomeVacina}</td>
                                        <td className="border px-4 py-2 border-black">
                                            <Link to={`/irl/vacina/${cadastro.id}`} className="text-blue-900 hover:text-blue-950 mx-1.5">
                                                Editar
                                            </Link>
                        ...
                        <div className="flex justify-end">
                            <Link to={`/irl/vacina`} className="block text-sm font-semibold">
