...
export default function ListaExame() {
    ...
    const fetchCadastros = async () => {
        try {
            const response = await instance1.get(`/exameFisicoP/${petId}`);
    ...
    const handleConfirmDelete = async () => {
        if (cadastroIdToDelete !== null) {
            try {
                const response = await instance1.delete(`/exameFisico/${cadastroIdToDelete}`);
    ...
    return (
        <div className="flex">
                <div className="flex flex-col mt-1 items-center w-full">
                    <div className=" bg-opacity-10 backdrop-filter backdrop-blur-md rounded-lg shadow-xl">
                        <div className="grid grid-cols-1 gap-4 mx-4">
                                    ...
                                    {currentCadastros.map((cadastro) => (
                                        <tr key={cadastro.id}>
                                            ...
                                            <td className="py-2 px-4 border border-black">
                                                <Link to={`/irl/exameFisico/${cadastro.id}`} className="text-blue-500 hover:text-blue-700">
                                                    Exibir
                                                </Link>
                            ...
                            <div className="flex justify-end">
                                <Link to={`/irl/exameFisico`} className="block text-sm font-semibold">
