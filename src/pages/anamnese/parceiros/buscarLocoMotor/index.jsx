...
export default function ListaLocoMotor() {
    ...
    const fetchCadastros = async () => {
        try {
            const response = await instance1.get(`/locomotorP/${petId}`);
    ...
    const handleConfirmDelete = async () => {
        if (cadastroIdToDelete !== null) {
            try {
                const response = await instance1.delete(`/locomotor/${cadastroIdToDelete}`);
                ...
                <div className="bg-slate-200 bg-opacity-10 backdrop-filter backdrop-blur-md rounded-lg shadow-xl">
                    <div className="grid grid-cols-1 gap-4 mx-4">
                                ...
                                {currentCadastros.map((cadastro) => (
                                    <tr key={cadastro.id}>
                                        ...
                                        <td className="py-2 px-4 border border-black">
                                            <Link to={`/irl/locomotor/${cadastro.id}`} className="text-blue-500 hover:text-blue-700">
                                                Exibir
                                            </Link>
                        ...
                        <div className="flex justify-end">
                            <Link to={`/irl/locomotor`} className="block text-sm font-semibold">
