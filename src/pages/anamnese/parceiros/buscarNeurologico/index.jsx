...
export default function ListaNeurologico() {
    ...
    const fetchCadastros = async () => {
        try {
            const response = await instance1.get(`/neuroP/${petId}`);
    ...
    const handleConfirmDelete = async () => {
        if (cadastroIdToDelete !== null) {
            try {
                const response = await instance1.delete(`/neuro/${cadastroIdToDelete}`);
                ...
                <div className="bg-slate-200 bg-opacity-10 backdrop-filter backdrop-blur-md rounded-lg shadow-xl">
                    <div className="grid grid-cols-1 gap-4 mx-4">
                                ...
                                {currentCadastros.map((cadastro) => (
                                    <tr key={cadastro.id}>
                                        ...
                                        <td className="py-2 px-4 border border-black">
                                            <Link to={`/irl/neurologico/${cadastro.id}`} className="text-blue-500 hover:text-blue-700">
                                                Exibir
                                            </Link>
                        ...
                        <div className="flex justify-end">
                            <Link to={`/irl/neurologico`} className="block text-sm font-semibold">
