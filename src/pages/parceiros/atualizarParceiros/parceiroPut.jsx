...
export default function ParceiroPut() {
    ...
    useEffect(() => {
        const fetchReceber = async () => {
            try {
                const response = await instance1.get(`/parceiro/${id}`);
                ...
                setNum(data.num);
                setBairro(data.pontoReferencia);
                setPontoReferencia(data.pontoReferencia);
                setTelefone_2(data.telefone_2);
                setCidade(data.cidade);
                setUf(data.uf);
                setObsEnd(data.obsEnd);
                setTipoId('');
                setTipoNome(data.tipos?.comercio || '');
                setRule(data.rule || '');
                setObs(data.obs);
            } catch (error) {
    ...
    const handleSubmit = async (event) => {
        ...
        const formattedCnpj = cnpj ? cnpj.replace(/[^\d]+/g, '') : null;  // Remove tudo que não for número
        const formattedCpf = cpf ? cpf.replace(/[^\d]+/g, '') : null;  // Remove tudo que não for número

        try {
            const response = await instance1.put(`/parceiro/${id}`, {
                cnpj: formattedCnpj,
                cpf: formattedCpf,
                status,
                email,
            ...
            <form key={formKey}>
                <div className="bg-slate-200 bg-opacity-10 backdrop-filter backdrop-blur-md rounded-lg shadow-xl ">
                    <div className="grid md:grid-cols-3 sm:grid-cols-1 gap-4  mx-2">
                         <SearchTipo tipoSel="Novo comercio:" onSelectTipo={(tipoId) => setTipoId(tipoId)} />
                        <CheckList
                            types="text"
                            descricao="Comercio:"
                            value={tipoNome}
                            onChange={setTipoNome}
                            readOnly
                            menssagemError=""
                        />
                        <CheckList
                        ...
                        <CheckList
                            ...
                            value={contato}
                            onChange={setContato}
                            menssagemError="O telefone do cliente não pode estar vazio"
                            inputComponent={
                                <InputMask
                                    ref={inputRef}
                                    mask="(99) 9 9999-9999"
                        ...
                        <div className="relative">
                            <CheckList
                                ...
                                value={cep}
                                onChange={setCep}
                                menssagemError="O CEP do cliente não pode estar vazio"
                                inputComponent={
                                    <InputMask
                                        ref={inputRef}
                                        mask="99999-999"
                        ...
                        <CheckList
                            types="text"
                            descricao="Endereço:"
                            placeholderInput="Joao mendes ferreira"
                            value={endereco}
                        ...
                        <CheckList
                            ...
                            menssagemError="O ponto de referência do cliente não pode estar vazio"
                        />
                        <CheckList
                            types="text"
