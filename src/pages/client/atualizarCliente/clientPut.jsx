...
export default function ClientePut() {
    ...
    useEffect(() => {
        const fetchReceber = async () => {
            try {
                const response = await instance1.get(`/cliente/${id}`);
                const data = response.data;

                setStatus(data.status);
                setEmail(data.email);
                setIndicacao(data.indicacao ? data.indicacao : "");
    ...
    const handleSubmit = async (event) => {
        ...
        if (!razaoSocial.trim()) {
            showWarningMessage('O nome do cliete é obrigatório!');
            return;
        ...
        try {
            const response = await instance1.put(`/cliente/${id}`, {
                status,
                email,
                indicacao,
                contato,
