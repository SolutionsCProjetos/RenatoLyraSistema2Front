  ...
  const fetchCadastros = async () => {
    try {
      ...
      const cadastrosComData = response.data.map((cadastro) => {
        const certData = cadastro.vencimentoCertificado
          ? cadastro.vencimentoCertificado.split("/")
          : null;
        const dataFormatada = certData
          ? `${certData[0]}/${certData[1]}/${certData[2]}`
          : "";

        const contraData = cadastro.vencimentoContrato
          ? cadastro.vencimentoContrato.split("/")
          : null;
        ...
        return {
          ...cadastro,
          vencimentoCertificado: dataFormatada,
          vencimentoContrato: contraFormatada,
        };
      });
  ...
  const searchFilters = () => {
    ...
    if (tempSearchTerm.trim() !== "") {
      filteredData = filteredData.filter((item) => {
        const itemData =
          `${item.razaoSocial} ${item.status} ${item.cpf} ${item.contato}`.toUpperCase();
    ...
    if (filterType === "dia" && selectedDates[0]) {
      ...
      filteredData = filteredData.filter((item) => {
        const certDateISO = formatDateToISO(item.vencimentoCertificado);
        const contractDateISO = formatDateToISO(item.vencimentoContrato);
      ...
      filteredData = filteredData.filter((item) => {
        ...
        return (
          (certDateISO >= startDate && certDateISO <= endDate) ||
