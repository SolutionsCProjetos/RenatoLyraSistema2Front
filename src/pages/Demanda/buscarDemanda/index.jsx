 ...
 */
    ...
    if (cadastroIdToDelete !== null) {
      try {
        ...
        if (error.response) {
          ...
          showErrorMessage(
            error.message || "Erro ao tentar a conexão com o banco!"
          );
        }
      } finally {
        closeModal();
  ...
  const formatTelefone = (telefone) => {
    ...
    if (tel.length === 11) {
      return `(${tel.slice(0, 2)}) ${tel[2]} ${tel.slice(3, 7)}-${tel.slice(
        7
      )}`;
  ...
  const fetchCadastros = async () => {
    try {
      ...
      const cadastrosComData = response.data.map((cadastro) => {
        const partesData = cadastro.dataSolicitacao.split("/");
  ...
  const searchFilters = () => {
    ...
    if (tempSearchTerm.trim() !== "") {
      filteredData = filteredData.filter((item) => {
        const itemData =
          `${item.protocolo} ${item.solicitante.nomeCompleto} ${item.status} ${item.solicitante.telefoneContato} ${item.solicitante.cpf} ${item.bairro}`.toUpperCase();
  ...
  const filtrarOperacoes = (operacoes, filtroStatus) => {
    ...
    if (filtroStatus === "Todos" || !filtroStatus) {
      return operacoes.filter((operacao) =>
        ["Pendente", "Aguardando Retorno", "Cancelada", "Concluída"].includes(
          operacao.status
        )
      );
    }
                  ...
                  {currentCadastros.map((cadastro) => (
                    <tr key={cadastro.id} className={`border-b `}>
                      ...
                      <td className="border px-4 py-2 border-black hidden sm:table-cell">
                        {cadastro.solicitante.nomeCompleto}
                      </td>
                      <td className="border px-4 py-2 border-black hidden sm:table-cell">
                        {formatTelefone(cadastro.solicitante.telefoneContato)}
                      </td>
                      <td className="border px-4 py-2 border-black hidden sm:table-cell">
                        {cadastro.prioridade}
                      </td>
                      <td className="border px-4 py-2 border-black hidden sm:table-cell">
                        {cadastro.dataSolicitacao
                          .split("-")
                          .reverse()
                          .join("/")}
                      </td>
                      <td className="border px-4 py-2 border-black ">
                        <Link to={`/irl/demanda/${cadastro.id}`}>
                          <BotaoEditarT />
                        </Link>
